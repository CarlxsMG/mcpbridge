...
The implementation column points at the **actual Prometheus metric names exposed by the
gateway** (see `GET /metrics` and `src/observability/metrics.ts`). The duration histogram
buckets are `[0.005, 0.01, 0.05, 0.1, 0.5, 1, 2, 5, 10, 30]` seconds.
...
| **SLI**                     | p99 of the `tools/list` MCP request handler duration, observed via the existing OTLP span `mcp.tools.list` (see `src/observability/tracing.ts`).                                                                                                               |
...
| **SLI**                | `sum(rate(http_requests_total{route=~"/admin-api/.*",status!~"5.."}[30d])) / sum(rate(http_requests_total{route=~"/admin-api/.*"}[30d]))` — using whatever HTTP metrics the gateway or its reverse proxy exports. |
...
…with the analogous `6h / 6` and `24h / 1` and `72h / 1` variants. Implement these in
Prometheus / Grafana; the gateway does not emit burn alerts itself.
