...
| `GATEWAY_PUBLIC_URL`                   | —                              | Public base URL the gateway advertises when generating client-connection configs (the admin UI's "Connect client" dialog / `gateway connect`). Optional — callers otherwise fall back to the browser origin or the CLI's `--url`.                                                 |
