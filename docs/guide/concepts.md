---
description: The vocabulary of MCP REST Bridge — clients, tools, shards, bundles, consumers, guards and the registry, as used across the docs and the admin UI.
---
...
| **[Bundle](/guide/bundles)** | An admin-curated, cross-client subset of tools (and/or composite macros) behind one endpoint — how you put several backends behind one MCP URL.                                                      |
...
| **Circuit breaker**          | Per-client failure protection (`closed → open → half_open`) that fails fast while a backend is unhealthy.                                                                                            |
...
- **Curate** what a client sees with serving modes and [bundles](/guide/bundles).
