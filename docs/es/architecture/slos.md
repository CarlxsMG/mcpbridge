...
Cada SLI de abajo nombra la **señal real de la que se mide** — una métrica Prometheus que
expone el gateway donde existe (ver `GET /metrics` y `src/observability/metrics.ts`), o una
ruta de audit / métrica de reverse-proxy donde se indique (SLO-3, SLO-4 y SLO-5 no son
métricas del gateway). Los buckets del histograma de duración son
`[0.005, 0.01, 0.05, 0.1, 0.5, 1, 2, 5, 10, 30]` segundos.
...
| **SLI**                      | p99 de la duración del handler MCP `tools/list`. **Aún no está instrumentado** — el tracer OTLP solo emite un span `tool_call <name>` por cada `tools/call` (`src/proxy/proxy.ts`); `tools/list` no tiene span ni métrica hoy. Este SLO es aspiracional hasta que ese handler esté instrumentado (o quede delante de una métrica de ruta del reverse-proxy). |
...
| **SLI**                  | `sum(rate(http_requests_total{route=~"/admin-api/.*",status!~"5.."}[30d])) / sum(rate(http_requests_total{route=~"/admin-api/.*"}[30d]))`. **El gateway no emite `http_requests_total`** — este SLI depende de un reverse proxy / ingress delante del gateway que exporte métricas HTTP por ruta, y no está cubierto por las reglas de `monitoring/` incluidas en el repo (solo SLO-1/2/6). |
