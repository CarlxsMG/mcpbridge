...
| ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Cliente** (backend)           | Un backend registrado — una API **REST** (tools descubiertas desde OpenAPI), una API **GraphQL** (`kind: "graphql"`, tools descubiertas desde el schema introspeccionado) o un **upstream MCP** (`kind: "mcp"`). |
...
| **Circuit breaker**             | Protección de fallos por cliente (`closed → open → half_open`) que falla rápido mientras un backend no está sano.                                                                                                |
