...
- **Un health-eviction hold también es por instancia, si enrutas con `/livez` en vez de
  ...
  readiness a `/livez` para que cada réplica sirva (consulta el
  [override del readiness-probe del Helm chart →](/es/guide/deployment#kubernetes-helm)), un
  re-registro de cliente puede aterrizar en una
