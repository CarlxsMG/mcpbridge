...
This is a chronological engineering log of MCP REST Bridge's Stryker mutation-testing
hardening program: per-file mutation scores, the tests added to close gaps, the
techniques that turned out to be reusable across files, and the equivalent-mutant
reasoning behind every score that tops out below 100%. It was moved out of
[`CHANGELOG.md`](./CHANGELOG.md) to keep that file user-facing — this log is the
detailed engineering record for anyone (human or agent) extending or re-verifying the
suite later, referenced from `CHANGELOG.md`'s `[Unreleased]` entry.

The program ran in numbered phases — a `P2` series hardening `src/security/*.ts` one
file at a time, followed by ten sequentially-numbered "domains" sweeping the rest of
the backend (`src/proxy`, `src/mcp`, `src/db`+`src/middleware`+`src/net`,
`src/tool-policies`+`src/tool-meta`+`src/content-filtering`+`src/backend-auth`,
`src/discovery`, `src/observability`, `src/routes`+`src/routes/admin`, `src/admin`,
and a final miscellaneous domain covering `src/lib`, `src/cli`, `src/catalog`,
`src/secrets`, `src/config*`, `ws-proxy.ts`, and the process entrypoints). Entries are
in the order the work happened; later entries assume the vocabulary and techniques
established earlier (equivalent-mutant classes, the `STRYKER_TEST_SCOPE` speedup, the
worktree-isolated parallel-Workflow pattern, and so on) without re-explaining them.
