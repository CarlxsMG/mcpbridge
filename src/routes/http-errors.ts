 ...
 *
 * `code` is `ErrorCode`, not `string`, on purpose: it makes ./error-codes.ts
 * the compile-time gate for every code that leaves through this helper — a new
 * or mistyped one fails `tsc` instead of shipping a code no consumer (and no
 * admin-UI translation) knows about.
 */
export function sendError(res: Response, status: number, code: ErrorCode, message: string): Response {
...
export function forbidden(res: Response, code: ErrorCode, message: string): Response {
