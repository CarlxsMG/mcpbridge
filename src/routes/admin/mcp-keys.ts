...
export const mcpKeyRoutes = Router();

mcpKeyRoutes.get("/mcp-keys", (req: Request, res: Response) => {
  res.status(200).json({ items: listMcpKeys().filter((k) => keyVisibleToCaller(req, k)) });
});

mcpKeyRoutes.post("/mcp-keys", requireAdminRole, (req: Request, res: Response) => {
...
mcpKeyRoutes.get("/mcp-keys/:id", (req: Request<{ id: string }>, res: Response) => {
...
mcpKeyRoutes.patch("/mcp-keys/:id", requireAdminRole, (req: Request<{ id: string }>, res: Response) => {
...
mcpKeyRoutes.post("/mcp-keys/:id/revoke", requireAdminRole, (req: Request<{ id: string }>, res: Response) => {
  const id = Number(req.params.id);
  const existing = getMcpKey(id);
  if (!existing) {
    notFound(res, "MCP_KEY_NOT_FOUND", "API key not found");
...
mcpKeyRoutes.delete("/mcp-keys/:id", requireAdminRole, (req: Request<{ id: string }>, res: Response) => {
  const id = Number(req.params.id);
  const existing = getMcpKey(id);
  if (!existing) {
    notFound(res, "MCP_KEY_NOT_FOUND", "API key not found");
