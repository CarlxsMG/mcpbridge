...
import { SEARCH_TOOL_NAME, type AdvertisedTool } from "./tool-search.js";
...
export function initComposites(): void {
  ...
  const rows = db.query(`SELECT name, description, enabled, input_schema_json FROM composite_tools`).all() as {
    name: string; description: string | null; enabled: number; input_schema_json: string;
  }[];
  ...
  for (const r of rows) {
    const steps = (stepStmt.all(r.name) as { target_client: string; target_tool: string; args_template_json: string }[]).map((s) => ({
...
export function listAdvertisedComposites(): AdvertisedTool[] {
  ...
  for (const [name, c] of liveComposites) {
    if (c.enabled) out.push({ name, description: c.description ?? `Composite tool: ${name}`, inputSchema: c.inputSchema });
  }
  return out;
}
...
export async function runComposite(
  ...
  if (!comp.enabled) return { isError: true, content: [{ type: "text", text: `Composite tool '${name}' is disabled` }] };
  if (comp.steps.length === 0) return { isError: true, content: [{ type: "text", text: `Composite tool '${name}' has no steps` }] };
  ...
  for (let i = 0; i < comp.steps.length; i++) {
    ...
    if (resolved === null || typeof resolved !== "object" || Array.isArray(resolved)) {
      return { isError: true, content: [{ type: "text", text: `Composite '${name}' step ${i + 1} produced non-object arguments` }] };
    }
    ...
    last = result;
    if (result.isError) {
      return { isError: true, content: [{ type: "text", text: `Composite '${name}' failed at step ${i + 1} (${target}): ${text}` }] };
    }
  }
...
export function listComposites(): CompositeSummary[] {
  ...
  return rows.map((r) => ({ name: r.name, description: r.description, enabled: r.enabled === 1, stepsCount: r.steps_count }));
}

export function getCompositeDetail(name: string): CompositeDetail | undefined {
  const db = getDb();
  const row = db
    .query(`SELECT name, description, enabled, input_schema_json, created_at, updated_at FROM composite_tools WHERE name = ?`)
    .get(name) as { name: string; description: string | null; enabled: number; input_schema_json: string; created_at: number; updated_at: number } | null;
  if (!row) return undefined;
  const steps = (db
    .query(`SELECT target_client, target_tool, args_template_json FROM composite_tool_steps WHERE composite_name = ? ORDER BY step_order`)
    .all(name) as { target_client: string; target_tool: string; args_template_json: string }[]).map((s) => ({
    targetClient: s.target_client,
    targetTool: s.target_tool,
    argsTemplate: JSON.parse(s.args_template_json) as Record<string, unknown>,
  }));
...
function refreshCache(name: string): void {
  ...
  if (!detail) { liveComposites.delete(name); return; }
  liveComposites.set(name, { enabled: detail.enabled, description: detail.description, inputSchema: detail.inputSchema, steps: detail.steps });
}
...
export async function createComposite(
  ...
  if (!isValidCompositeName(name)) {
    return { ok: false, error: { code: "INVALID_NAME", message: "Composite name must match /^[a-z0-9][a-z0-9_-]{0,62}$/ and not contain '__'" } };
  }
  if (typeof inputSchema !== "object" || inputSchema === null || (inputSchema as Record<string, unknown>).type !== "object") {
    return { ok: false, error: { code: "INVALID_SCHEMA", message: "inputSchema must be an object schema (type: object)" } };
  }
...
export async function updateComposite(
  name: string,
  updates: { description?: string | null; enabled?: boolean; inputSchema?: Record<string, unknown>; steps?: CompositeStep[] }
): Promise<CompositeMutationResult> {
  return withLock(name, async () => {
    const db = getDb();
    if (!db.query(`SELECT 1 FROM composite_tools WHERE name = ?`).get(name)) {
    ...
    if (updates.inputSchema !== undefined) {
      if (typeof updates.inputSchema !== "object" || updates.inputSchema === null || updates.inputSchema.type !== "object") {
        return { ok: false, error: { code: "INVALID_SCHEMA", message: "inputSchema must be an object schema (type: object)" } };
      }
    }
    ...
    const now = Date.now();
    const txn = db.transaction(() => {
      if (updates.description !== undefined) db.query(`UPDATE composite_tools SET description = ?, updated_at = ? WHERE name = ?`).run(updates.description, now, name);
      if (updates.enabled !== undefined) db.query(`UPDATE composite_tools SET enabled = ?, updated_at = ? WHERE name = ?`).run(updates.enabled ? 1 : 0, now, name);
      if (updates.inputSchema !== undefined) db.query(`UPDATE composite_tools SET input_schema_json = ?, updated_at = ? WHERE name = ?`).run(JSON.stringify(updates.inputSchema), now, name);
      if (updates.steps !== undefined) {
        db.query(`DELETE FROM composite_tool_steps WHERE composite_name = ?`).run(name);
        const insert = db.query(
          `INSERT INTO composite_tool_steps (composite_name, step_order, target_client, target_tool, args_template_json) VALUES (?, ?, ?, ?, ?)`
        );
        updates.steps.forEach((s, i) => insert.run(name, i, s.targetClient, s.targetTool, JSON.stringify(s.argsTemplate)));
    ...
    const scopeChanged = updates.enabled !== undefined || updates.inputSchema !== undefined || updates.steps !== undefined;
