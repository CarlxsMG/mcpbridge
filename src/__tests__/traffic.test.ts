...
describe("record / list / get / prune", () => {
  test("stores full args + a result preview; list & get read it back", () => {
    ...
    const items = listTraffic({});
    expect(items).toHaveLength(1);
    expect(items[0]).toMatchObject({ mcpToolName: "svc__get-x", clientName: "svc", toolName: "get-x", preview: "hello", isError: false });
  ...
  test("errorsOnly filter and prune", () => {
    recordTraffic({ mcpToolName: "svc__a", clientName: "svc", toolName: "a", keyId: null, args: {}, result: { content: [{ type: "text", text: "ok" }] }, durationMs: 1 });
    recordTraffic({ mcpToolName: "svc__b", clientName: "svc", toolName: "b", keyId: null, args: {}, result: { content: [{ type: "text", text: "bad" }], isError: true }, durationMs: 1 });
    expect(listTraffic({ errorsOnly: true })).toHaveLength(1);
    expect(listTraffic({ toolName: "a" })).toHaveLength(1);
    ...
    expect(listTraffic({})).toHaveLength(0);
  });
});
...
describe("proxy integration", () => {
  ...
  test("capture on: records args and a preview of the result", async () => {
    ...
    globalThis.fetch = (async () => new Response('{"r":1}', { status: 200, headers: { "content-type": "application/json" } })) as unknown as typeof fetch;
    await proxyToolCall(`${CLIENT}__get-x`, { q: "hi" });
    const items = listTraffic({});
    expect(items).toHaveLength(1);
