...
import { parseCron, cronMatches, createSchedule, listSchedules, setScheduleEnabled, deleteSchedule, runDueSchedules } from "../schedules.js";
...
describe("schedule evaluator", () => {
  ...
  test("a disabled schedule never fires", async () => {
    await reg();
    const s = createSchedule({ targetType: "client", clientName: "svc", action: "disable", cron: "30 14 * * *", actor: "t" });
  ...
  test("tool-scoped enable schedule flips just that tool", async () => {
    ...
    createSchedule({ targetType: "tool", clientName: "svc", toolName: "get-x", action: "enable", cron: "30 14 * * *", actor: "t" });
    expect(await runDueSchedules(AT_1430)).toBe(1);
  ...
  test("rejects an unknown client and a bad cron", async () => {
    await reg();
    expect(createSchedule({ targetType: "client", clientName: "ghost", action: "disable", cron: "* * * * *", actor: "t" })).toBe("INVALID_TARGET");
    expect(createSchedule({ targetType: "client", clientName: "svc", action: "disable", cron: "nope", actor: "t" })).toBe("INVALID_CRON");
  });
});
...
describe("schedules — admin route", () => {
  ...
  test("POST valid, list, PATCH, DELETE", async () => {
    ...
    const patch = await fetch(`${baseUrl}/admin-api/schedules/${id}`, { method: "PATCH", headers: bearer(), body: JSON.stringify({ enabled: false }) });
