...
describe("usage_spike alert integration", () => {
  ...
  test("fires the webhook when a spike is detected", async () => {
    ...
    app.post("/hook", (req, res) => { received = req.body as Record<string, unknown>; res.status(200).end(); resolveGot(); });
    await new Promise<void>((resolve) => {
      const srv = app.listen(0, "127.0.0.1", () => { server = srv; resolve(); });
    });
    const url = `http://127.0.0.1:${(server!.address() as AddressInfo).port}/hook`;

    createAlertRule({ name: "spike", eventType: "usage_spike", webhookUrl: url, threshold: 3, minCalls: 20, actor: "t" });
