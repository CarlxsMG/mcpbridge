...
<template>
  <button type="button" class="toggle" :class="on ? 'toggle-on' : 'toggle-off'" :disabled="disabled" @click="emit('click')">
    {{ on ? onLabel : offLabel }}
  </button>
</template>

<style scoped>
.toggle { display: inline-flex; align-items: center; gap: 0.45em; border-radius: var(--radius-pill); padding: 0.28rem 0.8rem; font-size: 0.78rem; font-weight: 600; cursor: pointer; background: var(--surface); transition: background-color 0.12s ease; }
