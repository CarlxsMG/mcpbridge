...
| **SLO-3** tools/list latency     | _not instrumented_ — the OTLP tracer only emits a `tool_call <name>` span per `tools/call`; `tools/list` gets no span or metric | ❌ aspirational: needs the `tools/list` handler instrumented (or a reverse-proxy route metric), not `/metrics`                       |
